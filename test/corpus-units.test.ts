...
describe("enumerateUnits", () => {
  ...
  it("emits sqpack+golden+mtrl+tex for every pack, plus pmp for .pmp packs", () => {
    const pmpCount = packs.filter((p) =>
      p.toLowerCase().endsWith(".pmp"),
    ).length;
    expect(units.length).toBe(packs.length * 4 + pmpCount);
  });
  ...
  it("is deterministic and sorted by pack path, fixed check order per pack", () => {
    ...
    // per pack, the checks appear in [sqpack, golden, mtrl, tex, (pmp)] order
    for (const pack of packOrder) {
      ...
      const expected = pack.toLowerCase().endsWith(".pmp")
        ? ["sqpack", "golden", "mtrl", "tex", "pmp"]
        : ["sqpack", "golden", "mtrl", "tex"];
