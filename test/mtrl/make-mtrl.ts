...
/**
 * A pre-Dawntrail EW colorset .mtrl (same shape as buildMinimalMtrl -- 256-entry raw colorset +
 * dye, character.shpk, one NormalMap0 sampler -- so doesMtrlNeedDawntrailUpdate flags it and
 * upgradeMaterial runs the colorset branch) PLUS a trailing empty sampler (index 255, same
 * placeholder shape as buildEmptySamplerMtrl below). upgradeColorsetMaterial never removes or
 * touches placeholder textures, so the placeholder survives into the upgraded mtrl and
 * serializeMtrl's empty-sampler guard (mtrl/serialize.ts:43-47) still fires on write-back --
 * exercising the fix-round-2 requirement that this port-gap propagates out of materialRound
 * instead of being swallowed by its per-material catch (upgrade.ts).
 */
export function buildEmptySamplerColorsetMtrl(): Uint8Array {
  // Strings: "test.tex\0" @0 (9), "uv1\0" @9 (4), "character.shpk\0" @13 (15) = 28 (already %4).
  const stringBlockSize = 28;
  const shaderNameOffset = 13;
  ...
  b.u16(1); // shaderKeyCount
  b.u16(1); // shaderConstantsCount
  b.u16(2); // textureSamplerCount (real NormalMap0 + trailing empty sampler)
  b.u16(0x0011); // materialFlags
  b.u16(0x0022); // materialFlags2
  ...
  // Shader-constant descriptor: id, offset 0, size 4.
  b.u32(0xcafebabe).u16(0).u16(4);
  // Sampler: NormalMap0 on texture index 0 (real texture, gives upgradeColorsetMaterial its
  // resolvable normal, EndwalkerUpgrade.cs:912-921).
  b.u32(SAMPLER_NORMAL_MAP_0).u32(0x00010203).u8(0).bytes([0, 0, 0]);
  // Sampler: ColorMap0 on texture index 255 -- out of range, so parse creates a fake placeholder
  // texture to hold it (same shape as buildEmptySamplerMtrl). ColorMap0's raw id never matches an
  // exact-id scan (findSpecDiffuse looks for g_SamplerSpecular/g_SamplerDiffuse specifically), so it
  // cannot interfere with the colorset branch's other sampler lookups.
