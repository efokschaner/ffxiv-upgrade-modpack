...
describe("readPmp case-insensitive Files resolution", () => {
  ...
  // TexTools resolves this via a case-insensitive NTFS read (PMP.cs:1178); we must too.
  ...
  // normalization) is TOLERATED, not fatal: LoadPMP does no existence check (PMP.cs:159) and
  // UnpackPmpOption builds a FileStorageInformation whose RealPath simply does not exist
  // (PMP.cs:1169-1200). The entry STAYS in the option — the upgrade rounds gate on
...
describe("readPmp Windows path-normalization (trailing dots/spaces)", () => {
  ...
  // an NTFS Path.Combine read (PMP.cs:1178) after a LoadPMP that never checks existence (PMP.cs:159).
  function buildPmp(
    gamePath: string,
    displayEntry: string,
    filesValue: string,
...
// Port of LoadPMP's ExtraFiles scan (PMP.cs:278-280): a zip member neither referenced by any
// option's Files value nor a manifest json (IsPmpJsonFile, PMP.cs:293-306) is preserved as an
// "extra" (preview images, readmes, ...) rather than silently dropped.
describe("readPmp ExtraFiles (PMP.cs:278-280)", () => {
  const gamePath =
    "chara/equipment/e0834/material/v0001/mt_c0201e0834_top_a.mtrl";
  ...
  it("DOES treat a member referenced only via a trailing-dot Files value as an extra too (PMP.cs:261/:274/:279)", () => {
    // TexTools' allPmpFiles (PMP.cs:261/:274) is the RAW Files value, `.ToLower()`'d only — no
    // trailing dot/space trim — compared (PMP.cs:279) against the on-disk relative path, which NTFS
    // already trimmed by the time LoadPMP's ExtraFiles scan runs (PMP.cs:78 unzips first). So a
    ...
    // ...AND also captured as an ExtraFile, per the untrimmed PMP.cs:279 comparison. The archive
