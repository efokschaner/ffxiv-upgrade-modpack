...
citation-and-reading discipline plus corpus widening (`docs/BACKLOG.md` prioritized item 3). A
C#-side coverage harness was considered and rejected — we deliberately split modules, and a large
fraction of C# branches (every `files == null` half of `EndwalkerUpgrade.cs`) is structurally
unreachable on the modpack path, so the signal would arrive buried in noise.
