...
malformed placeholders now dropped at load) → **316 today** after the metadata round
...
included in the 316). Remaining diffs are the deferred rounds and gaps: source-`.tex`
resize/trailing-trim (T2/T3), the `e0208` override (T4), and container-manifest
structure. The BC-source-decode ±1 is not a baselined diff but a confirmed intended
divergence via the single programmatic `DIVERGENCE_RULES` entry. End state: every
baseline empty (byte-zero), with the committed allow-list holding only whatever narrow
paths prove empirically to resist parity (see §8.2 — the texture round is now targeted
byte-exact, not assumed divergent).

### 8.4 Path to a complete end-to-end tool & known gaps

**Remaining path.** A shippable end-to-end upgrader is: transforms complete
(round 6, which drives the ratchet the rest of the way to zero — rounds 3→4→5 are
shipped) **plus** the UI (round 7) wired
