...
- **Round 4 — Texture round.** Port of `UpgradeRemainingTextures`: consumes the
  ...
  **Update 2026-07-10 — shipped.** All four usages generate; `.tex` ratchet burned down
  ...
  T2**. The `index-path-overrides` table is missing some base-game entries (**BACKLOG
  T4**). Spec: `2026-07-09-texture-round-design.md`.
...
source-`.tex` resize/trailing-trim (T2/T3), the index-path overrides (T4), and
container-manifest structure. The BC-source-decode ±1 is not a baselined diff but a
confirmed intended divergence via the single programmatic `DIVERGENCE_RULES` entry. End
...
- **Two bundled reference tables have an unfaithful *silent* miss.** *(Added
  2026-07-20, from a completeness audit of all eight tables.)* AGENTS.md requires a
  bundled table to **be** the existence oracle, so that a lookup miss means the file is
  genuinely absent. Six tables satisfy this — `imc-table` (15,695 roots) and `est-table`
  enumerate their full domain and **throw** on the unfaithful case; `hair-materials`
  (1,513) and `eye-materials` (339) enumerate the full race grid against the real 040000
  index, so their silent skip *is* the C# `FileExists == false` branch; `eye-base-textures`
  has no lookup surface. The other two do not, and both fail **silently**, which makes
  them the sharpest robustness risk in the project:
  - `index-path-overrides` (11 entries) is **corpus-derived** — its extractor walks
    `test/corpus/inputs`, not the game — and falls back to the naming convention on a
    miss (`material.ts:144-147`). A base-game material the corpus never saw gets a wrong
    `_id.tex` path with no signal. `docs/backlog/2026-07-10-index-path-overrides-e0208.md`.
  - `hair-texture-exists` is **namespace-scoped** (hair/zear/tail textures, id ≤ 500) but
    is asked about sampler paths that may point anywhere, answering a hard `false` and
    silently suppressing a rename.
    `docs/backlog/2026-07-20-hair-texture-exists-namespace-scope.md`.

  Neither is catchable by the ratchet, **by construction** — it can only ever see inputs
  the local corpus already contains. That is the general shape of the residual risk now:
  code coverage is high (92.98% lines / 84.6% branches), so what remains is unexercised
  **data and inputs**, not unexercised code paths.
