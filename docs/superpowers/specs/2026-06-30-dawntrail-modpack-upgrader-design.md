...
**~421 today** after the texture + TexFix rounds (the `.tex` bulk generated byte-exact;
malformed placeholders now dropped at load). Remaining diffs are the deferred rounds and
gaps: `.meta` (metadata round), source-`.tex` resize/trailing-trim (T2/T3), the `e0208`
override (T4), and container-manifest structure. *(Three real coverage packs — veil /
Marcellus / Balloon — were added during round 4; their baselined diffs are on top of the
~421 and fall into these same categories.)* The BC-source-decode ±1 is not a baselined
diff but a confirmed intended divergence via the single programmatic `DIVERGENCE_RULES`
entry. End state: every baseline empty (byte-zero), with the committed allow-list holding
only whatever narrow paths prove empirically to resist parity (see §8.2 — the texture round is now
