...
- **Round 6 — Partials + reference bundling.** *Shipped (specs dated 2026-07-15 →
  ...
  **Update 2026-07-31:** `hair-texture-index.ts`/`hair-texture-exists.ts` are replaced by
  `chara-index.ts` (generated by `scripts/extract-chara-index.ts`)/`file-exists.ts`'s
  `fileExists`, a **complete** `040000` oracle over all of `chara/` (333,072 entries across
  82,369 folders) rather than a namespace-scoped one — see
  `docs/superpowers/specs/2026-07-31-game-file-exists-oracle-design.md`. This closes the open
  follow-up noted just below.
  ...
  **`includePartials` is deliberately not plumbed** — partials always run. No shipped
  ...
  `docs/backlog/2026-07-20-hair-texture-exists-namespace-scope.md`.~~ **Shipped 2026-07-31** —
  see the Update note above and `docs/superpowers/specs/2026-07-31-game-file-exists-oracle-design.md`.
  Item file deleted per `docs/BACKLOG.md` convention.
...
- **Two bundled reference tables had an unfaithful *silent* miss; one is now fixed.**
  *(Added 2026-07-20, from a completeness audit of all eight tables; updated same day
  ...
  - ~~`hair-texture-exists` is still **namespace-scoped** (hair/zear/tail textures, id ≤
    ...
    the item-seeded enumeration pattern above as its template.~~ **Shipped 2026-07-31.**
    Replaced by `fileExists` (`src/upgrade/reference/file-exists.ts`) over a complete,
    unfiltered `chara-index.ts` (333,072 entries, 82,369 folders) — a genuine miss now
    means the file is absent, not out-of-namespace, and a valid FFXIV path the table
    can't resolve throws (`UnportedGapError`) rather than answering a silent `false`.
    Five call sites moved onto it: `repath-hair-mashups.ts`'s nine, the index-path
    steal's gate B (which also retired `idTexExists`/`ID_TEX_PACKED`), and the hair/eye
    existence gates in `unclaimed-hair.ts`/`eye-mask.ts`, now split from their content
    lookups. See `docs/superpowers/specs/2026-07-31-game-file-exists-oracle-design.md`.
    Item file deleted per `docs/BACKLOG.md` convention.

  Both tables are now complete, closing this bullet's open half; the remaining residual
  risk is the general shape noted above: code coverage is high (92.98% lines / 84.6%
  branches), so what remains is unexercised **data and inputs**, not unexercised code
  paths.
