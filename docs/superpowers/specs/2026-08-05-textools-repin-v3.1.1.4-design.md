...
Filed: 2026-08-05 · Status: **implemented** on `feat/textools-repin-v3.1.1.4` — all six §14 exit
criteria met, all 11 commits carry a §10 verdict. (Row 2's Combining refusal was the one `ported`
row implemented on that branch, by operator ruling 2026-08-07.) **Part B — row 1's `Tex.cs` mip work,
the last verdict still owing an implementation — landed 2026-08-09 on `feat/textools-repin-part-b`**;
see §10 row 1 and the final row of §10.1's totals table. Every §10 verdict is now discharged, so this
spec is closed.
...
`initializeData`, and `traceListenerConfigured` (`oracle.ts:326`) substring-matches the absolute
`UPGRADE_TRACE_LOG` value, which `oracle.ts:53` builds from `homedir()`. Having the setup script
compute and write it removes a step that is easy to get subtly wrong by hand.

Without it every `/upgrade` run dies at `assertUpgradeTraceListenerConfigured` (`oracle.ts:340`),
...
|---|--------|------|---------|---------------------|---------|
| 1 | `1993bf6` | 2025-11-02 | Be less strict about texture mip data, and fix non-ascending lodmips | `Tex.cs` +9/−10 | **`bug register` + `ported` — IMPLEMENTED 2026-08-09 on `feat/textools-repin-part-b`.** All three hunks landed: (a) and (c) in `14e9194`, (b) in `1c25308`, pinned against the real oracle by two purpose-built synthetic packs, `test/corpus/synthetic/load-seam-mipfix.ttmp2` (`d3b120d`) and `load-seam-npot.ttmp2` (`8e77407`). The register entry is reconciled to the §9 fixed-upstream form and its "What Part B owed" itemisation is marked discharged line by line. Corpus effect, measured under the bless: **zero bytes moved** on all 166 pre-existing baseline files — see §10.1's totals table for why that is the expected result and not a null one. The rest of this cell is the pre-implementation analysis, kept as the record of *why*; read the verdict against §8's taxonomy: `ported` classifies the **hunk** (it changes ported behaviour). It was deliberately not labelled `deferred`, which §8 reserves for work ruled out of scope and which §4.5 would require operator sign-off for. Three hunks, all in `Tex.cs`, all landing in symbols we port. (a) `TexHeader.ToBytes` loses its **entire** validation block (`Tex.cs:138-145` *at the old pin*, +0/−9). §9's summary named only the `LoDMips` ordering guard; `git show` shows the commit also deletes `LoDMips[2] >= MipCount`, `MipFlag > 15` and `MipCount > 13` — `ToBytes` is now a pure serializer. We port all four verbatim as `assertTexHeaderWritable` (`src/tex/header.ts:159`), called at `src/upgrade/validate-tex.ts:49` where a throw **drops the file at the load seam**; Part B deletes the function and that call. (b) `CreateTexFileHeader`'s LoD2 line becomes `newMipCount > 2 ? 2 : (newMipCount - 1)` (`Tex.cs:1126`); ours still writes `: 0` (`src/tex/header.ts:79`) and reaches real output via `encodeUncompressedTex` (`src/tex/encode.ts:105`), so every regenerated exactly-2-mip texture moves bytes. (c) `FixUpBrokenMipOffsets` gains an ascending-order clamp in its LoDMips loop (`Tex.cs:203-219`, running `maxLodMip`, sets `modified`); ours has only the `>= mipCount` clamp (`src/tex/header.ts:135`). Bug register reconciled: **#19 fixed upstream** by (a)+(b) — status line added, and note the landed (b) is behaviourally identical to the fix #19 already proposed for every reachable `newMipCount >= 1`; **#20 audited, UNAFFECTED** — it lives in `EndwalkerUpgrade.cs`, untouched across the entire range; **#21 audited, NOT fixed** — `TexHeader` is still a `struct` (`:71`), still taken by value (`:159`), and both `header.MipCount` writes are still to the local copy (`:173`, `:200`). **Reach re-measured 2026-08-09** (pre-Part-B fact-check; detail in `docs/TEXTOOLS_BUGS.md` #19's *Reach re-measured*, which is authoritative). Two corrections to this row, neither changing the three edits Part B owes. (i) **The prediction above — "every regenerated exactly-2-mip texture moves bytes" — is technically true and practically vacuous.** `mipCount == 2` out of `generateMipmaps` (`src/tex/encode.ts`) requires a minimum dimension of exactly 4, `resizeForMerge` (`src/upgrade/texture.ts`) refuses <64 for non-BC7 so Branch A cannot produce one, and at the time of this note (2026-08-09, pre-execution), every `buildCanonicalTexHeader` call under `scripts/generate-synthetics/` and `test/` passed `mipCount = 1` — so no synthetic rebuilds and no cached golden was invalidated by (b). (That stopped being true within this same branch — `build-synthetic-load-seam-mipfix.ts` and `-npot.ts` both pass `mipCount = 2`+, and the npot builder leaves the derived `LoDMips` unmasked, so its NPOT fixture's input header now carries `[0,1,1]`; harmless, but see `docs/TEXTOOLS_BUGS.md` #19's *Reach re-measured*, which is the authoritative, dated version of this claim.) Expect corpus movement from **(c)**, the ascending clamp, which newly sets `modified` on a Branch-B texture that previously returned `null` or threw; §7.2's re-bless discipline applies unchanged, but attribute movement to (c), not (b). (ii) **The deleted guard had a second call site on the same load path**: `TTMP.cs · FixOldTexData · 1438` calls `Tex.CompressTexFile` unconditionally, and its `.tex` arm calls `header.ToBytes()` (`Tex.cs:1324`), so at the OLD pin a *healthy* two-mip `.tex` in an old pack was dropped even when `ValidateTexFileData` returned null. We do not port the recompress, so we kept those files — a pre-existing opposite-direction divergence from the old oracle that this commit retroactively makes correct. It adds no port work, but Part B owes the rationale rewrite in `src/upgrade/load-fixes.ts`. |
| 2 | `76535f4` | 2026-05-24 | Add PMP Combining group import support | `PMP.cs` +93/−3, `WizardData.cs` +19/−1 | **`ported`** — **operator ruling 2026-08-07: `ported`, not `deferred`, and implemented on THIS branch rather than deferred to Part B**, resolving Task 9's §4.5 escalation of this row. *What upstream did:* the commit registers `[JsonSubtypes.KnownSubType(typeof(PMPCombiningGroupJson), "Combining")]` (`PMP.cs:1494`) and adds `PMPCombiningGroupJson` (`:1555-1603`, whose `Options` override returns `OptionData`, `:1565`) plus `PmpCombiningOptionJson` (`:1701-1703`) / `PmpCombiningContainerJson` (`:1705-1707`), so the base `PMPGroupJson.Options` virtual and its `NotImplementedException($"Unimplemented PMP group type: {Type}")` (`:1517`) is no longer reached for `"Combining"` and such a pack now LOADS. It is refused one stage later instead: `EGroupType` gains `Combining` (`WizardData.cs:32-37`), `WizardGroupEntry.GroupType` resolves `ModOption is PMPCombiningGroupJson → Combining` (`:611-625`), and `WizardGroupEntry.ToPmpGroup`'s FIRST statement throws `InvalidDataException("Editing or exporting PMP Combining groups is not supported.")` (`:897-900`) — reached from `WizardData.WritePmp`'s group-assembly loop (`:1613`), never from the default-mod absorption search (`:1581`), whose predicate is `g.GroupType == EGroupType.Standard` (`:1576`). So the OUTCOME is unchanged (refuse) but the STAGE (load → write) and the MESSAGE both moved, which matters because `assertMatchedUpgradeFailure` substring-matches our thrown text against the oracle's trace. *Measured, not inferred* (ConsoleTools /upgrade v3.1.1.4, 2026-08-08): a probe pack carrying a Combining group beside a genuinely-upgrading `.mtrl` exits −1 with **no output file** and a trace reading `System.IO.InvalidDataException: Editing or exporting PMP Combining groups is not supported.` at `WizardGroupEntry.<ToPmpGroup>` ← `WizardData.<WritePmp>` ← `<WriteModpack>` ← `ModpackUpgrader.<UpgradeModpack>` ← `ConsoleTools.<HandleUpgrade>`; the byte-identical probe WITHOUT the Combining group exits 0 and writes a pack. (The upgrading `.mtrl` is load-bearing: `ModpackUpgrader.cs:244-247` only writes when `AnyChanges`, and a Combining group carries no upgradable data, so a Combining-only pack no-ops and never reaches the seam.) *What we ported:* (a) `KNOWN_PMP_GROUP_TYPES` (`src/container/manifest-types.ts`) now accepts `"Combining"`, the minimum that lets the group reach the writer — its `Containers` ride along untyped in `PmpGroupJson`'s index signature and are never interpreted; (b) `writePmp`'s group-assembly loop (`src/container/pmp.ts`) throws the **byte-identical** message at the point corresponding to `ToPmpGroup`'s first statement — a plain `Error`, deliberately NOT `UnportedGapError`, because it reproduces a throw the C# itself performs rather than signalling a gap in our port; (c) the three `GroupType != EGroupType.Standard` tests the C# already had (`:1532-1535` option loop, `:1576` absorption predicate, and the option-prefix pass-1 filter) now go through a new `groupType()` port of `WizardGroupEntry.GroupType` (`src/container/group-type.ts`) so a Combining group is excluded exactly where an Imc group is. A **zero-option** Combining group is still dropped silently at load by `FromPMPGroup`'s `return null` (`:857-861`) — TexTools upgrades that pack, and so do we. *Evidence:* synthetic corpus pack `test/corpus/upgrade-error/pmp-combining-group.pmp` (`scripts/generate-synthetics/build-synthetic-pmp-combining.ts`), which the `/upgrade` expected-failure harness runs against the real oracle — it required extending `registerUpgradeCheck`'s error branch to span the WRITE as well as load+transform, since ConsoleTools' `/upgrade` writes too (`ModpackUpgrader.cs:244-247`) and a write-seam refusal was otherwise unmatchable. Plus fresh-clone unit coverage in `test/container/manifest-types.test.ts` (read accepts) and `test/container/pmp-write.test.ts` (write refuses, and the zero-option boundary). *Explicitly NOT ported:* Combining SUPPORT, and with it two load-time details of the upstream commit — `FromPMPGroup` leaving a Combining option's `StandardData`/`ImcData` null (`:822-843`) and `LoadPMP`'s ExtraFiles scan seeding `allPmpFiles` from `Containers[].Files` (`PMP.cs:236-250`). Both are unobservable because every write path refuses first, and both are recorded at the read seam in `src/container/pmp.ts`. The commit's remaining `PMP.cs` hunks stay outside our path: the import branch is in `ImportPMP`, the `Groups.Any(x => x.Type == "Combining") → return null` early-out is in `UnpackPMP` (`:1059-1062`; `UnpackPMP` starts `:1042` — the simple-modpack path), and `WizardData.HasCombiningGroups` (`WizardData.cs:1103-1109`) has no consumer in either vendored repo. |
...
| 6 | `d09cd2b` | 2026-07-19 | Don't crash on v4 import | `PMP.cs` +25/−5 | `ported` — `default_mod.json` becomes optional, gated on `File.Exists(defModPath)` (`PMP.cs:182`); ported at `src/container/pmp.ts:218-225` (the read seam, which leaves `defaultMod` null for an absent member rather than throwing). |
...
| after `1993bf6` (row 1) *(+2 corpus packs: 76/90/166 → 78/92/170)* | 3379 | 2476 | 0 | 5855 |

**What moved, in three parts — the row above is a rise, and every entry of it is an addition rather
than a regression.** (i) **Pre-existing packs: nothing at all.** All 166 baseline files present before
the bless were byte-identical after it — none changed, none removed — verified by hashing a pre-bless
snapshot of all three directories file by file. This is the expected result and the reason §10 row 1's
*Reach re-measured* was written: hunk (b) reaches output only for a regenerated texture whose smaller
dimension is exactly 4, which nothing in the corpus produces, and hunk (c)'s ascending clamp turns out
to fire on no corpus texture either. **A zero-movement result was the predicted legitimate outcome
here** — this commit's value is the class-1 fix (a broken-offset non-ascending-`LoDMips` `.tex` is now
repaired and kept instead of silently dropped at our load seam), not a diff reduction. (ii) **The two
new packs: +4 baseline files, +46 entries** (`load-seam-mipfix` 14 upgrade + 10 resave;
`load-seam-npot` 13 upgrade + 9 resave), which is the whole of the +27/+19 rise. The pack-count column
rises by 4 rather than 2 because it counts baseline *files* and each pack has both an `upgrade` and a
`resave` one. 44 of the 46 are the pre-existing `ModsJsons[].{Name,Category,DatFile}` re-derivation
gap (`docs/backlog/2026-07-13-resave-ttmp2-name-category.md`) that every
real-golden `.ttmp2` synthetic hits — cosmetic, unrelated to this branch. `load-seam-mipfix`'s entries
are *all* of that kind: its two repaired `.tex` payloads are byte-identical to the golden, which is
the actual proof this row is about. (iii) **`load-seam-npot`'s two payload entries are a real defect,
not cosmetic** — the remaining 2 of the 46, one per baseline, both
`chara/equipment/e9997/texture/c9997e9997_top_b_d.tex`, `detail: 21920 vs 21924 bytes`. This pack's
first real-oracle run **found a previously-unknown port gap**: at `ValidateTexFileData`'s Branch A,
`MergePixelData` is the *final* encode (no later `ConvertToDDS`), so TexTools emits a full
`GetMipCount` pyramid — 7 levels for a 64px result — where we emit `CreateFast8888DDS`'s 2px-floor
chain of 6. mip0 is byte-identical; the divergence begins exactly at the mip0/mip1 boundary. It
**predates this branch** and is a finding, not a regression. Documented before the bless, per
AGENTS.md's rule that a baseline suppresses a diff rather than confirming one, as a new section of
`docs/backlog/2026-07-22-bc-encoder-merge-pixel-data.md` (same root cause: `MergePixelData`'s encode
is unported) with its index entry updated to match. Anyone burning that baseline down should treat
those two entries as that item and the other 44 as manifest noise.

`roundtrip` stayed at **0**, verified across the bless against a pre-bless snapshot (the directory is
empty before and after — the goal state).

One row is appended per ported commit as the total moves — usually down, but see the row-1 note above
for a case where it rises because the corpus grew. The PMP-v4 row above is a **re-keying**
...
The pack-by-pack proof **for the PMP-v4 row**, recorded here rather than cited out to a transient
report. Exactly **three**
baseline files changed under that bless — one `upgrade`, two `resave` — matching the three pre-bless
...
**Settled, 2026-08-08 — that guess was wrong.** The hunks do reach us: the commit moves where a
Combining group is REFUSED (load → write) and changes the message, which
`assertMatchedUpgradeFailure` compares against the oracle's trace. Recorded as `ported` in §10
row 2, per the operator's 2026-08-07 ruling; implemented on this branch. Carrying the group
opaquely via `raw` remains true of everything BELOW the refusal — we accept the `Type`, never
model `Containers`, and throw at the write seam.
