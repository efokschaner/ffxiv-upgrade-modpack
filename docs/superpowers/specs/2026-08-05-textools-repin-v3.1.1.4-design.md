...
criteria met, all 11 commits carry a §10 verdict. **Part B is the follow-on** for the `ported`
verdicts whose *implementation* §10 assigns to it: row 1's `Tex.cs` mip work, itemised in
`docs/TEXTOOLS_BUGS.md` #19's "What Part B owes". (Row 2's Combining refusal was the one `ported`
row implemented on this branch instead, by operator ruling 2026-08-07.)
