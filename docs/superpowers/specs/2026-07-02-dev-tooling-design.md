...
- A commit that violates formatting/lint rules is caught locally by a **pre-commit** hook; a push that breaks tests is caught by a **pre-push** hook.
...
- **GitHub Actions / CI** — explicitly deferred by operator; not set up in this work. (The pre-push hook is therefore the enforcement gate for tests, see §4.)
...
- **pre-push** (heavier; the test enforcement gate given no CI):
  - `npm test` (full suite via the existing runner).

Hooks are advisory-bypassable with `--no-verify` for genuine escape hatches; they are not a security control.

**Rationale for split:** the corpus tests are non-trivial in wall-clock (a parallel runner was purpose-built for them), so the full suite belongs at push time, not on every commit. With CI deferred, pre-push is the last automated gate before shared history.
...
## 6. Components and interfaces summary
...
- `README.md` — short "Development" section: the `check` workflow, hooks, and the `blame.ignoreRevsFile` opt-in.
...
2. Isolated commit A: `biome check --write .` — the one-time reflow. Verify `tsc`/`test` green, diff is format-only.
3. Isolated commit B: add `.git-blame-ignore-revs` with commit A's SHA + README note.
4. Add `lefthook` + `lefthook.yml` + `prepare` script + `.editorconfig`; verify hooks fire (a deliberately-misformatted staged file is blocked by pre-commit; a failing test is blocked by pre-push).
...
- No pre-commit test execution (kept at pre-push to avoid per-commit pain).
