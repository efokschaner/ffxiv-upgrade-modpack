...
**Interfaces:**
- Produces the generated module exporting: `INDEX_DROP_PACKED: string`, `INDEX_KEEP_PACKED: string`
...
export function resolveStolenIndexPath(materialPath: string): string | undefined {
  ...
  if (KEEP.has(key)) return reconstructIndexPath(materialPath, true) ?? undefined;
  if (DROP.has(key)) return reconstructIndexPath(materialPath, false) ?? undefined;
  return undefined;
}
```
...
- **Type consistency:** `resolveStolenIndexPath` / `idTexExists` / `reconstructIndexPath` /
  `INDEX_DROP_PACKED` / `INDEX_KEEP_PACKED` / `INDEX_EXCEPTIONS` / `ID_TEX_PACKED` used consistently across
