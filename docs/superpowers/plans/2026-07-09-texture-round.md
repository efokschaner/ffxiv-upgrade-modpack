...
git commit -m "docs(plan): texture-round corpus coverage assessment"
```

---

### Task 12: Synthetic modpacks for under-covered usages
...
**Interfaces:** produces a synthetic pack under `test/corpus/synthetic/` that flows through the `/upgrade` golden harness.

- [ ] **Step 1: Identify the gaps to synthesize**

From Task 11, pick the usages with thin/zero coverage (expected: hair — 1 pack today; gear masks — verify). For each, the synthetic must use **power-of-two, equal-size** source textures so it is byte-exact (no resize). Do NOT synthesize an NPOT/resize case here — that belongs to the resampler follow-up.
...
- [ ] **Step 3: Build the pack and run the harness**
...
- [ ] **Step 4: If it does not fully match, diagnose**
...
- §6 golden-first + coverage assessment + synthetics + unit tests → Tasks 10, 11, 12, plus unit tests throughout. ✓
