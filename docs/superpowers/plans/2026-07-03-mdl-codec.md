...
| 13 | boneSets | **version-dependent**: v6 `md.boneSetSize·2 + md.boneSetCount·4` (Mdl.cs:741); v5 `132·md.boneSetCount` (fixed 64×i16+i32 per set, Mdl.cs:779-797 — real v5 files have `boneSetSize=0`) |
...
After section 21 the running offset must **not exceed** `MDL_HEADER + VertexInfoSize + ModelDataSize` (the geometry start); any remaining bytes up to it are carried as an opaque `trailing` slice (usually empty — see the parse code and spec §2).
...
src/mdl/parse.ts        CREATE: parseMdl(bytes, filePath?) — the structural walk + no-overrun assert + trailing slice  (Task 3)
...
export interface MdlSections {
  ...
  trailing: Uint8Array; // opaque bytes between boundingBoxes and geometry (usually empty; §2)
}
...
export function buildMinimalMdl(version: number): Uint8Array {
  const md: MdlModelData = {
    ...
    boneSetSize: 64, // v5: 132·count=132; v6: 64·2+1·4=132 (both 132-byte block for this fixture)
    unknown13: 0,
    patch72TableSize: 0,
    unknown15: 0,
    unknown16: 0,
  ...
  const sections = [
    ...
    filler(
      version >= 6 ? md.boneSetSize * 2 + md.boneSetCount * 4 : 132 * md.boneSetCount,
      8,
...
describe("parseMdl structural walk", () => {
  ...
  it("slices each section at its computed length", () => {
    ...
    expect(s.trailing).toHaveLength(0); // minimal fixture: sections land exactly on modelDataSize
    expect(mdl.geometry).toHaveLength(16);
  });

  it("parses a v6 file (version-dependent bone-set slicing)", () => {
  ...
  it("captures a trailing gap opaquely (modelDataSize > named sections)", () => {
    // Grow modelDataSize by 48 so 48 geometry bytes fall inside the model-data block as `trailing`.
    const grown = new Uint8Array(buildMinimalMdl(5)); // copy so we don't mutate a shared buffer
    const dvg = new DataView(grown.buffer);
    dvg.setUint32(8, dvg.getUint32(8, true) + 48, true);
    expect(parseMdl(grown).sections.trailing).toHaveLength(48);
  });
});
```
...
export function parseMdl(bytes: Uint8Array, filePath = ""): XivMdl {
  ...
  // Bone-set block length is VERSION-DEPENDENT (§5): real v5 files store boneSetSize=0 and use fixed
  ...
  // Any bytes between the last named section and the geometry are carried opaquely (§2): the reference
  // GetXivMdl seeks to VertexDataOffset for geometry and tolerates a gap (Mdl.cs:1000-1027) — some mods
  // carry an extra 32*BoneCount per-bone bounding-box block an older TexTools/Penumbra writer appended
  // (current writer no longer does) but GetXivMdl reads past.
...
Run: `npm test`
Expected: PASS. The `[mdl] <pack>: N byte-exact, M legacy-skipped` lines appear; no `overran modelDataSize` errors. An over-read means a named section's length math is wrong (usually a garbage downstream count) — fix it against `Mdl.cs`, do NOT loosen the no-overrun assertion. (Under-reads are absorbed by the `trailing` slice by design — see spec §2.)
...
- **Spec §2 approach (slice walk + no-overrun assertion + opaque `trailing` slice):** Task 3 (`parseMdl`).
...
- **Spec §5 version-dependent bone-set slicing (v6 `BoneSetSize·2 + BoneSetCount·4`; v5 `132·BoneSetCount`):** Task 3 (`boneSets`), exercised v5+v6 in Tasks 3-4 and over the corpus in Task 5.
