...
| `HAS_EXTRA_MESHES` | `0x10` | `EMeshFlags2` bit that adds a 60-byte extra-mesh header |
...
| 4 | lodHeaders | `60 · 3` |
| 5 | extraMeshHeader | `60` if `md.flags2 & 0x10`, else `0` |
...
export function parseMdl(bytes: Uint8Array, filePath = ""): XivMdl {
  ...
  const extraMeshHeader = r.readBytes((md.flags2 & HAS_EXTRA_MESHES) !== 0 ? 60 : 0);
