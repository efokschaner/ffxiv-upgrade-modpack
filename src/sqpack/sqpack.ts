...
export function decodeSqPackFile(entry: Uint8Array): DecodedFile {
  ...
  switch (type) {
    case SqPackType.Standard:
      return { type, data: decodeType2(entry) };
    case SqPackType.Model:
      return { type, data: decodeType3(entry) };
    case SqPackType.Texture:
      return { type, data: decodeType4(entry) };
    default:
      throw new Error(`sqpack: unsupported entry type ${type}`);
  }
}

/** Compress already-uncompressed bytes into a SQPack entry of the given type. */
export function encodeSqPackFile(
  data: Uint8Array,
  type: SqPackType,
): Uint8Array {
  switch (type) {
    case SqPackType.Standard:
      return encodeType2(data);
    case SqPackType.Model:
      return encodeType3(data);
    case SqPackType.Texture:
      return encodeType4(data);
    default:
      throw new Error(`sqpack: unsupported type ${type}`);
  }
}
