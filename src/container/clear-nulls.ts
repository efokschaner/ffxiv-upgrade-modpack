...
// 1253-1285) and the two HasData predicates it reads: WizardGroupEntry.HasData (· 627-633) and
// WizardPageEntry.HasData (· 980-986).
//
// WizardGroupEntry.HasData is `Options.Any(x => x.HasData)` (:627-633), so it depends in turn on
// WizardOptionEntry.HasData (:259-280), which short-circuits on its FIRST line:
// `if (_Group.ModOption != null) { return true; } // "Read mode."`. `ModOption` is assigned in
// exactly two places in the whole file — `WizardGroupEntry.FromWizardGroup` (:655) and
// `WizardGroupEntry.FromPMPGroup` (:773) — and never reset, and those are the ONLY group
...
export function clearNulls(pages: ModpackPage[]): void {
  // `DataPages.ToList()` (:1255) — iterate a SNAPSHOT while removing from the live list.
  for (const p of [...pages]) {
    // :1258 — `p.FolderPath = null`. Not ported: `ModpackPage` carries no `folderPath` field at all
    ...
    if (!pageHasData(p)) {
      pages.splice(pages.indexOf(p), 1); // :1261
      continue;
    }
    // :1265-1272 — same snapshot-then-remove shape, at the group level. This check IS null-guarded
    // in the C#; only the page-level one above is not.
    for (const g of [...p.groups]) {
      if (g === null || !groupHasData(g)) {
        p.groups.splice(p.groups.indexOf(g), 1);
      }
      // :1273 — g.FolderPath = null. Our group folder paths live in optionPrefixes' local Maps
      // (rebuilt per call), so there is no stored field to null here.
    }
    // :1275-1282 — `if (o == null) g.Options.Remove(o)` is NOT ported: ModpackOption has no null
