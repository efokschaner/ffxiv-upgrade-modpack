...
//
// ---------------------------------------------------------------------------------------------
// WHAT ITS BASELINE CONTAINS — and, more importantly, what it does NOT
// ---------------------------------------------------------------------------------------------
...
// are byte-identical to the golden's. There is NO .tex entry in either of this pack's baseline
// files, and there must never be one — that absence is the assertion, and a payload entry appearing
// here later means the Branch-B repair has regressed away from upstream 1993bf6's fixed behaviour.
// What the baseline does hold (14 fields on /upgrade, 10 on /resave, blessed 2026-08-09) is
// entirely ModsJsons[].{Name,Category,DatFile}: the same pre-existing, already-registered writer gap
// every other real-golden synthetic .ttmp2 hits — docs/backlog/2026-07-13-resave-ttmp2-name-category.md:
// TexTools RE-DERIVES Name/Category from the game path on write (resolving this fixture's
// chara/equipment/e9998 path to a fictitious but path-shape-valid item) where we round-trip the
// source's placeholder for untouched files, or emit "" for regenerated ones (the mask upgraded in
// place, and the created _id.tex). Confirmed by diffing our own upgradeModpack() output against the
// extracted golden manifest field-by-field — see build-synthetic-imc-weapon.ts's header for the
// first synthetic pack to hit this same gap.
